...
class PredictClassTest < Test::Unit::TestCase
  ...
  def test_check_compare_edges_2
     e1 =  Edge.new("adj-edge", 1)
  ...
  def test_check_compare_review_with_patterns
     tc = TextPreprocessing.new
     patterns = tc.read_patterns("unit/automated_metareview/patterns-test-1.csv", @pos_tagger)
  ...
  def test_check_predict_class_with_test1_patterns_file
     pattern_files_array = ["unit/automated_metareview/patterns-test-1.csv"]
  ...
  def test_check_predict_class_with_test2_file
    pattern_files_array = ["unit/automated_metareview/patterns-test-2.csv"]
    review_text = ["The sweet potatoes in the vegetable bin are green with mold."]   
    content_instance = PredictClass.new
    g.generate_graph(review_text, @pos_tagger, @core_NLP_tagger, false, true)
    content_probs = content_instance.predict_classes(@pos_tagger, @core_NLP_tagger, review_text, g, pattern_files_array, pattern_files_array.length)
  ...
  def test_check_predict_class_with_multiple_pattern_files
    #s = File.read(File.join(RAILS_ROOT, "app/assets/some_text_file.txt"))
    pattern_files_array = ["unit/automated_metareview/patterns-test-1.csv",
                            "unit/automated_metareview/patterns-test-2.csv"]
    review_text = ["The sweet potatoes in the vegetable bin are green with mold."]   
    content_instance = PredictClass.new
    g.generate_graph(review_text, @pos_tagger, @core_NLP_tagger, false, true)
    content_probs = content_instance.predict_classes(@pos_tagger, @core_NLP_tagger, review_text, g, pattern_files_array, pattern_files_array.length)
  ...
  def test_check_compare_reviews_with_patterns_2
    tc = TextPreprocessing.new
    patterns = tc.read_patterns("unit/automated_metareview/patterns-test-2.csv", @pos_tagger)
    reviews = ["The sweet potatoes in the vegetable bin are green with mold."]
    g.generate_graph(reviews, @pos_tagger, @core_NLP_tagger, false, true)    
    content_instance = PredictClass.new
