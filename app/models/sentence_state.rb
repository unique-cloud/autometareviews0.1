...
#    puts("**** Inside identify_sentence_state #{str_with_pos_tags}")
    ...
    #converted if/else to ternary operator
...
#------------------------------------------#------------------------------------------
  ...
  def sentence_state(str_with_pos_tags)
    ...
    (st.length-1).times do |k| #Replaced for loop with times loop
      ps = st[k]
    ...
    prev_negative_word = ''
    (i-1).times do |j| #changed for loop to times loop
      ...
      elsif j+1 < count && !tokens[j].nil? && !tokens[j+1].nil? && is_negative_phrase(tokens[j]+' '+tokens[j+1]) == NEGATED
      ...
      elsif j+1 < count && !tokens[j].nil? && !tokens[j+1].nil? && is_suggestive_phrase(tokens[j]+' '+tokens[j+1]) == SUGGESTIVE
        returned_type = SUGGESTIVE
        j = j+1
...
def is_negative_word(word)
  ...
  #for i in (0..NEGATED_WORDS.length - 1) #Changed for loop with foreach
  NEGATED_WORDS.each do |nw|
    if word.casecmp(nw) == 0
...
def is_negative_descriptor(word)
  not_negated = POSITIVE
  #for i in (0..NEGATIVE_DESCRIPTORS.length - 1)  #Changed for loop with foreach
  NEGATIVE_DESCRIPTORS.each do |nd|
    if word.casecmp(nd) == 0
      not_negated =  NEGATED #indicates negation found
      break
    end  
  end
...
def is_negative_phrase(phrase)
  not_negated = POSITIVE
  #for i in (0..NEGATIVE_PHRASES.length - 1) #Changed for loop with foreach
...
def is_suggestive(word)
  ...
  #for i in (0..SUGGESTIVE_WORDS.length - 1) #Changed for loop with foreach
  SUGGESTIVE_WORDS.each do |w|
    if word.casecmp(w) == 0
...
def is_suggestive_phrase(phrase)
  not_suggestive = POSITIVE
  #for i in (0..SUGGESTIVE_PHRASES.length - 1) #Changed for loop with foreach
  SUGGESTIVE_PHRASES.each do |p|
    if phrase.casecmp(p) == 0
      not_suggestive =  SUGGESTIVE #indicates negation found
      break
    end
  end
