...
class TopicSentenceIdentification
  ...
  def find_topic_sentences(clusters, sentence_similarity)
    if(clusters.length == 1 && clusters[0].sent_counter == 1) #only one cluster with one sentence
      clusters[0].topic_sentences = Array.new
...
#    puts "clusters.length #{clusters.length} before"
    cg.calculate_sentence_similarities_within_cluster(clusters, sentence_similarity)
#    puts "clusters.length #{clusters.length} after"
...
#      puts "clust_sents.length #{clust_sents.length}"
#      puts "clust_sents.avg_similarity #{clust_sents[0].avg_similarity}"
